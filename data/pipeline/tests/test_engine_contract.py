...
#: La grieta que T-04 abrió —cinco constituyentes que el motor soportaba y el dataset seguía
#: descartando— **se cerró en T-13 al regenerar**. Lo que queda es la aserción que la vigilaba, sin
#: excepción que la relaje: si el dataset vuelve a descartar algo que el motor entiende, es deriva
#: nueva entre catálogo y dataset y sale en rojo aquí.


def test_the_dataset_uses_every_constituent_the_engine_understands() -> None:
    """Ningún JSON puede descartar un constituyente que el motor sí sabe sintetizar.

    Es la aserción que en T-04 hubo que relajar con una lista de cinco nombres (`EP2`, `MA2`, `MB2`,
    `MKS2`, `2MS6`) porque el dataset se había generado con el catálogo anterior. T-13 regeneró y la
    lista se borró: descartar hoy uno de esos cinco es tirar precisión a la basura teniéndola.
    """
    ...
    assert not (dropped_in_dataset & ENGINE_CONSTITUENTS)
