...
2. **Los cuatro «no» tienen que ser distinguibles**: no hay ítem, el ítem no es ese taxón, el ítem
   no tiene `P18`, y ninguna de sus `P18` acredita autor o licencia. Los cuatro acaban en `sinFoto`
   con motivo, y **ninguno interrumpe la ingesta**.
...
def test_un_fichero_sin_autor_no_esta_completo() -> None:
    """Falta el autor → la foto no se puede publicar, y se dice **qué** falta."""
    crudo = json.loads(_imageinfo())
...
def test_sin_item_en_wikidata_no_hay_foto_y_se_dice(monkeypatch: pytest.MonkeyPatch) -> None:
    vacia = json.dumps({"search": [], "success": 1}).encode()
    pedidas = _red(monkeypatch, {commons.url_busqueda(NOMBRE): vacia})
    resultado = commons.resolver(NOMBRE)
    assert resultado.desenlace == commons.SIN_ITEM
    assert "no tiene ningún ítem" in resultado.motivo
    assert len(pedidas) == 1, "no se sigue preguntando por un taxón que no tiene ítem"
...
def test_las_urls_son_las_que_se_midieron_contra_la_fuente() -> None:
    ...
    for campo in (commons.LICENCIA_CORTA, commons.ARTISTA, commons.LICENCIA_URL):
